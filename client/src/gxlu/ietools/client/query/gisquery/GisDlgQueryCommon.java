...
import gxlu.ietools.client.query.QueryConditionParseIFC;
import gxlu.ietools.client.query.QueryConditionParseImpl;
import gxlu.ietools.client.query.QueryItem;
import gxlu.ietools.client.query.QueryItems;
...
public class GisDlgQueryCommon extends UIGisBaseQueryDialog implements ChildrenProvider, QueryMethodInterface,
OperatorMethodInterface{
	...
	public GisDlgQueryCommon(Frame owner, String path, String title) throws ContainerException, ClassNotFoundException
	{
					...
					else
					{
						...
						if (vecDict.size() > 0)
						{
							c = QueryControlFactory.createListQueryControl(queryItem.getTitle(),
								new String[] { queryItem.getName() }, queryItem.getDclass(), queryItem.getDattr(),
								UIGisBaseQueryDialog.MULTI_SELECT);
							// c.setIDString("ID_" + queryItem.getName());
						}
						else
						{
										...
										if (vecDict.size() > 0)
										{
											try
											{
												v.addElement(FieldDictionary.getFieldDesc(queryItem.getDclass(),
													queryItem.getDattr(), Byte.parseByte(m.invoke(object).toString())));
											}
											catch (NumberFormatException e)
											{
												v.addElement("");
								...
								else if (queryItem.getType().equalsIgnoreCase("ParentQuery"))
								{
									...
									if(queryItem.getName()!=null&&queryItem.getName().indexOf("[")>=0){
										...
										if(resultObj!=null&&resultObj.size()>0){
											bObject=(YObjectInterface)resultObj.get(0);
										}else{
											bObject=null;
										}
									}else{
										bObject = (YObjectInterface) m.invoke(object);
									}
									if (bObject == null)
									{
										v.addElement(null);
	...
	public Vector execQuery(QueryParam param) throws SDHException
	{
		...
		String strBclass=param.getObjectClass().toString();
		if(strBclass.charAt(strBclass.lastIndexOf(".")+1)=='Y'){
			vResult=GisQueryClient.exeGisQuery((YObjectInterface)ReflectHelper.newInstance(strBclass, null, null),param);
		}
	...
	public UniQueryChildInterface getQueryChild(byte childID)
	{
		...
		if (hmChildItemSet != null && hmChildItemSet.get(b) != null)
		{
			...
			if (obj instanceof UIGisBaseQueryDialog)
			{
				dlg = (UIGisBaseQueryDialog) obj;
